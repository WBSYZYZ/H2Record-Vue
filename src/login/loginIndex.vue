  ...
  <div>
      ...
      <Renderer ref="renderer" pointer resize="window">
        ...
        <Scene>
          <Points ref="points" :position="{ z: -150 }">
            ...
            <ShaderMaterial
              :props="{ blending: 2, depthTest: false, uniforms: uniforms, vertexShader: vertexShader, fragmentShader: fragmentShader }">
              <Texture :src="loginIndexImg" uniform="uTexture" />
            </ShaderMaterial>
          </Points>
        </Scene>
        <EffectComposer>
    ...
    <div class="login-box">
      ...
      <form novalidate>
        ...
        <div class="user-box">
          <input type="text" name="" required="" v-model="code" @input="checkSendMsgFlag"/>
          <label>请输入验证码</label>
        </div>
        <button v-if="sendMsgFlag" @click="sendCode" type="button" :disabled="disabled">
          <span></span>
          <span></span>
          <span></span>
          <span></span>
        ...
        <button v-else @click="login" @mouseenter="targetTimeCoef = 100" @mouseleave="targetTimeCoef = 1" type="button">
          <span></span>
          <span></span>
          <span></span>
          <span></span>
...
export default {
  ...
  data() {
    return {
      ...
      radio: "",
      sendMsgFlag: true,
      loginIndexImg: require("../assets/loginIndex.jpg"),
      niceColors: [
        [
          "#69d2e7",
          "#a7dbd8",
  ...
  setup() {
    ...
    function login() {
      ...
      async function loginPush() {
        const response =await axios
          .post("/dashBoard/user/login", form)
      ...
      if (response.status == 200) {
        if(response.data.success==false){
          ...
          for (let i in response.data.data) {
            cookies.set(i, response.data.data[i], { expires: 2 });
            
          }
          // 跳转到首页
          router.push({
                path:"/indexDashboard",//目标路由地址
            });
        } else {
          ElMessage.error(response.data.errorMsg);
        }
    ...
    function sendCode() {
      ...
      async function sendCodePush(){
        ...
        .post("/dashBoard/user/code?phone=" + form.phone)
        .catch((err) => {
          ElMessage.error(err);
        });
        if(reponse.data.success==true){
          alert("由于本站未备案故无法发送验证码，您的验证码为:"+reponse.data.data);
        }else{
          if(reponse.data.errorMsg !=undefined){
            // 禁用按钮
            disabled.value = true;
            // 按钮倒计时
            console.log(reponse.data.errorMsg)
            let i = reponse.data.errorMsg;
            console.log(i)
...
`
    ...
    const niceColors = ['#a70267', '#f10c49', '#fb6b41', '#f6d86b', '#339194']
    const palette = niceColors
    ...
    return {
      ...
      login,sendCode,...toRefs(form),codeBtnMsg,disabled,form

    }
  },
  mounted() {
    const pageInstance = getCurrentInstance();
    const renderer = pageInstance.refs.renderer
    const positionN = renderer.three.pointer.positionN
    const points = pageInstance.refs.points.points
  ...
  methods: {
    ...
    updateColors() {
      ...
      const palette = this.niceColors[ip];
...
@import url("../login/css/style.css");
</style>